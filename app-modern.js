...
class SADEModern {
    ...
    async loadRankingData() {
        try {
            ...
            console.log('✅ Dados do ranking carregados com sucesso!');
        } catch (error) {
            console.error('❌ Erro ao carregar ranking:', error);
            this.showNotification('Erro ao carregar dados do ranking', 'error');
        }
    }
    ...
    updateRankingDisplay() {
        ...
        const selectedYear = document.getElementById('ranking-year-filter').value;
        
        // Atualizar estatísticas
        this.updateRankingStats(selectedYear);
        
        // Atualizar tabela
        this.updateRankingTable(selectedYear);
        
        // Atualizar gráficos
        this.updateRankingCharts(selectedYear);
    }

    updateRankingStats(selectedYear) {
        let allData = [];
        
        if (selectedYear === 'all') {
            // Combinar dados de todos os anos
            Object.values(this.rankingData).forEach(yearData => {
                allData = allData.concat(yearData);
            });
        } else {
            allData = this.rankingData[selectedYear] || [];
        }

        // Calcular estatísticas
        const totalSchools = new Set(allData.map(item => item.escola)).size;
        const totalStudents = allData.reduce((sum, item) => sum + item.total_alunos_geral, 0);
        const avgPerformance = allData.length > 0 ? 
            (allData.reduce((sum, item) => sum + item.media_geral, 0) / allData.length).toFixed(1) : 0;
        const bestPerformance = allData.length > 0 ? 
            Math.max(...allData.map(item => item.media_geral)).toFixed(1) : 0;

        // Atualizar DOM
        document.getElementById('ranking-total-schools').textContent = totalSchools;
        document.getElementById('ranking-total-students').textContent = totalStudents.toLocaleString();
        document.getElementById('ranking-avg-performance').textContent = avgPerformance;
        document.getElementById('ranking-best-performance').textContent = bestPerformance;
    }

    updateRankingTable(selectedYear) {
        ...
        if (selectedYear === 'all') {
            // Mostrar dados de todos os anos agrupados
            Object.entries(this.rankingData).forEach(([year, yearData]) => {
                yearData.forEach(item => {
                    data.push({...item, ano: year});
                });
            });
            // Reordenar por média geral
            data.sort((a, b) => b.media_geral - a.media_geral);
            data.forEach((item, index) => item.posicao = index + 1);
        } else {
            data = this.rankingData[selectedYear] || [];
        }

        tbody.innerHTML = data.map(item => {
            const medalClass = item.posicao <= 3 ? 
                `bg-${item.posicao === 1 ? 'warning' : item.posicao === 2 ? 'secondary' : 'warning'} text-white` : '';
            
            return `
                <tr class="${item.posicao <= 3 ? 'table-warning' : ''}">
                    <td class="text-center">
                        <span class="badge ${medalClass} fs-6">
                            ${item.posicao}º
                            ${item.posicao <= 3 ? '<i class="fas fa-trophy ms-1"></i>' : ''}
                        </span>
                    </td>
                    <td class="fw-bold">${item.escola}</td>
                    <td class="text-center">
                        <span class="badge bg-light text-dark">${item.turma || '-'}</span>
                    </td>
                    <td class="text-center">
                        <span class="badge bg-primary">${item.materias.LP?.toFixed(1) || '-'}</span>
                    </td>
                    <td class="text-center">
                        <span class="badge bg-success">${item.materias.MAT?.toFixed(1) || '-'}</span>
                    </td>
                    <td class="text-center">
                        <strong class="text-info">${item.media_geral.toFixed(1)}</strong>
                    </td>
                    <td class="text-center">
                        <span class="badge bg-info">${item.total_alunos_geral}</span>
                    </td>
                </tr>
            `;
        }).join('');
    ...
    updateRankingCharts(selectedYear) {
        let data = [];
        ...
        this.createRankingBarChart(data.slice(0, 10)); // Top 10

        // Gráfico de pizza das disciplinas
        this.createRankingPieChart(data);
    }
    ...
    createRankingBarChart(data) {
        ...
        this.charts.ranking = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: data.map(item => `${item.escola} ${item.turma || ''}`.trim()),
                datasets: [{
                    label: 'Média Geral',
                    data: data.map(item => item.media_geral),
                    backgroundColor: [
                        '#FFD700', '#C0C0C0', '#CD7F32', // Ouro, Prata, Bronze
                        ...Array(7).fill('#17a2b8') // Azul para o resto
                    ],
                    borderColor: [
                        '#FFA500', '#A9A9A9', '#8B4513',
                        ...Array(7).fill('#138496')
                    ],
                    borderWidth: 2
                }]
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Top 10 Melhores Desempenhos'
                    }
                },
                scales: {
                    y: {
    ...
    createRankingPieChart(data) {
        const ctx = document.getElementById('rankingSubjectChart');
        if (!ctx) return;

        // Destruir gráfico anterior se existir
        if (this.charts.rankingSubject) {
            this.charts.rankingSubject.destroy();
        }

        // Calcular médias por disciplina
        const lpSum = data.reduce((sum, item) => sum + (item.materias.LP || 0), 0);
        const matSum = data.reduce((sum, item) => sum + (item.materias.MAT || 0), 0);
        const lpAvg = lpSum / data.length;
        const matAvg = matSum / data.length;

        this.charts.rankingSubject = new Chart(ctx, {
            type: 'doughnut',
            data: {
